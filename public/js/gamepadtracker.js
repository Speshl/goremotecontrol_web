class GamePadTracker {
    ...
    commandFromXbox(myGamepad) {
        let command = this.neutralCommand;
        //esc
        if(myGamepad.buttons[6].value > .01 &&  myGamepad.buttons[6].value >= myGamepad.buttons[7].value){
            //brake
            command[0] = this.midPosition - this.mapToRange(myGamepad.buttons[6].value, .01, 1, this.minPosition, this.midPosition);
        }else if(myGamepad.buttons[7].value > .01){
            //gas
            command[0] = this.mapToRange(myGamepad.buttons[7].value, .01, 1, this.midPosition, this.maxPosition);
        }else{
            //neutral
            command[0] = this.midPosition;
        }
        //servo
        let steerCommand = command[1];
        if(myGamepad.axes[0] > .01){
            steerCommand = this.mapToRange(myGamepad.axes[0], .01, 1, this.midPosition, this.maxPosition);
        }else if(myGamepad.axes[0] < -.1){
            steerCommand = this.mapToRange(myGamepad.axes[0], -1, -.01, this.minPosition, this.midPosition);
        }else{
            steerCommand = this.midPosition;
        }
        ...
        if(myGamepad.axes[2] > .01 || myGamepad.axes[2] < -.01){
            this.panPos += this.mapToRange(myGamepad.axes[2], -1, 1, -1*this.panSpeed, this.panSpeed);
        }
        if(myGamepad.axes[3] > .01 || myGamepad.axes[3] < -.01){
