...
func CreateRecievePipeline(payloadType webrtc.PayloadType, codecName string, device string, volume string) *RecievePipeline {
	...
	case "opus":
		pipelineStr += fmt.Sprintf(", payload=%d, encoding-name=OPUS ! rtpopusdepay ! decodebin ! pulsesink volume=%s", payloadType, volume) // device=%s  Put back device after testing with default
	// case "vp9":
